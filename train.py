...
if __name__ == '__main__':
    ...
    pretrainEpochs = 5  # 预训练5个epoches
    trainEpochs = 3  # 正式训练3个epoches
    optimizer = torch.optim.Adam(net.parameters())
    batch_size = 128
    crossEnt = torch.nn.BCELoss()  # 二元交叉熵做平均

    
    ## Initialize data loaders
    trSet = ngsimDataset('data/TrainSet.mat')
    valSet = ngsimDataset('data/ValSet.mat')
    trDataloader = DataLoader(trSet,batch_size=batch_size,shuffle=True,num_workers=8,collate_fn=trSet.collate_fn)  # 使用collate_fun可以自己操作每个batch的数据
    ...
    for epoch_num in range(pretrainEpochs+trainEpochs):
        ...
        for i, data in enumerate(trDataloader):
            ...
            # hist.shape = [16, 128, 2]
            # nbrs.shape = [16, 825, 2]  这是一个变化值
            # mask.shape = [128, 3, 13, 64]
            # lat_enc.shape = [128, 3]
            # log_enc.shape = [128, 2]
            # fut.shape = [25, 128, 2]
            # op_mask.shape = [25, 128, 2]
